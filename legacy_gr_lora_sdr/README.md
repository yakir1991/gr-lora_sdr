...
- Clone this repository
	...
	```
- Go to the legacy module directory
        ```sh
        cd gr-lora_sdr/legacy_gr_lora_sdr/
        ```
...
#### Usage
- An example of a LoRa transmitter and receiver can be found in examples/ (both python and grc).
...
- The OOT blocks doesn't appear in gnuradio-companion:	
	- The new blocks can be loaded in gnuradio-companion by adding the following lines in home/{username}/.gnuradio/config.conf (If this file doesn't exist you need to create it):
		```
		[grc]
                local_blocks_path=path_to_the_downloaded_folder/gr-lora_sdr/legacy_gr_lora_sdr/grc
## Changelog
